...
from cpt import conditional_log_likelihood, cpt_p_pearson_torch
...
class sEMGDataset(Dataset):
  def __init__(self, X, Y, C, index, sid, train=False):
    ...
    # using a custom sampler is way too much work at the moment, might as well
    # generate the entire sequence all at once and just use the unshuffled index
    # to pass along
    if self.train:
      # 1. calculate the total amount of batches that are needed
      # 2. sample the indices of training data based on their subject IDs
      self.idx = range(len(self.Y))
      print(self.idx)
...
if __name__ == "__main__":
  ...
  for sub_test in range(0, 1):
    ...
    dsets_train = sEMGDataset(X_Train[splits[0],:], Y_Train[splits[0]], C_Train[splits[0]], splits[0], ID_Train[splits[0]], train=True)
    dsets_valid = sEMGDataset(X_Train[splits[1],:], Y_Train[splits[1]], C_Train[splits[1]], splits[1], ID_Train[splits[1]], train=False)
